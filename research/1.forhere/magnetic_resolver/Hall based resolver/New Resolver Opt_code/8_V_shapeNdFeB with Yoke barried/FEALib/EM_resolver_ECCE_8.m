...
%%
v_shape=1; %if want to use v shape magnet

global HandleToFEMM
pole_number=6;
D_arc_1=36;
% % % %
D_arc_2=D_arc_1*0.95;  % should use greater than 0.8, 0.8~0.99
deg_arc_1=20; % should use 10~120
deg_arc_2=50; % should use 10~40
m_in_1=2; % should use 1~3
slit_width=11; % should use width 5~10
m_thickness=3; % should use 1~3
m_gap=0.2;
b_th_2=1; % use 1 to 3
m_in_2=4;
...
bridge_arc_ratio=0.87; % to grap the point for arc
...
if deg_arc_2>0
    if m_in_1>m_in_2
        mi_addarc(slit_width/2-x_plus_1-x_plus_2,(D_arc_1/2)-m_in_1-y_plus_1-y_plus_2,x_bridge_2,y_bridge_2,deg_arc_2,5);        
    ...
    if m_in_1<m_in_2
     ...
     if m_in_1>m_in_2
        mi_addarc(x_bridge_2,y_bridge_2,slit_width/2-x_plus_1-x_plus_2,(D_arc_1/2)-m_in_1-y_plus_1-y_plus_2,-deg_arc_2,5);
...
mi_clearselected();
mi_addblocklabel(slit_width/2,(D_arc_1/2)-m_thickness-m_in_1+0.1);
mi_selectlabel(slit_width/2,(D_arc_1/2)-m_thickness-m_in_1+0.1);
mi_setblockprop('Air', 0, 1, 'None', 0, 0, 0);
mi_clearselected();
...
if v_shape==1
    ...
    for z=2:2:pole_number
        ...
        mi_addblocklabel(((D_arc_1/2)-m_in_1-m_thickness*0.5-(m_in_2-m_in_1)*0.5)*cos(deg_temp_2+(30+(z-1)*360/pole_number)*2*pi/360),((D_arc_1/2)-m_in_1-m_thickness*0.5-(m_in_2-m_in_1)*0.5)*sin(deg_temp_2+(30+(z-1)*360/pole_number)*2*pi/360));
        mi_selectlabel(((D_arc_1/2)-m_in_1-m_thickness*0.5-(m_in_2-m_in_1)*0.5)*cos(deg_temp_2+(30+(z-1)*360/pole_number)*2*pi/360),((D_arc_1/2)-m_in_1-m_thickness*0.5-(m_in_2-m_in_1)*0.5)*sin(deg_temp_2+(30+(z-1)*360/pole_number)*2*pi/360));
        mi_setblockprop('NdFeB',1,0,'None',180-deg_temp*180/pi+30+(z-1)*360/pole_number,2,1);
    ...
    for z=1:1:pole_number
        ...
        mi_setblockprop('Air', 0, 1, 'None', 0, 3, 0);
        mi_clearselected();
...
temp_hall_x=(D_arc_1/2+airgap);
temp_hall_y=2.2 %width/2 of the hall sensor
hall_depth=1.6 %detph of the hall sensor
angle_hall=atan2(temp_hall_y,temp_hall_x)
mi_addnode((D_arc_1/2+airgap)*cos(angle_hall+pi/2),(D_arc_1/2+airgap)*sin(angle_hall+pi/2))
mi_addnode(-(D_arc_1/2+airgap)*cos(angle_hall+pi/2),(D_arc_1/2+airgap)*sin(angle_hall+pi/2))
mi_addnode((D_arc_1/2+airgap)*cos(angle_hall+pi/2),(D_arc_1/2+airgap)*sin(angle_hall+pi/2)+hall_depth)
mi_addnode(-(D_arc_1/2+airgap)*cos(angle_hall+pi/2),(D_arc_1/2+airgap)*sin(angle_hall+pi/2)+hall_depth)
mi_addsegment((D_arc_1/2+airgap)*cos(angle_hall+pi/2),(D_arc_1/2+airgap)*sin(angle_hall+pi/2),(D_arc_1/2+airgap)*cos(angle_hall+pi/2),(D_arc_1/2+airgap)*sin(angle_hall+pi/2)+hall_depth)
mi_addsegment(-(D_arc_1/2+airgap)*cos(angle_hall+pi/2),(D_arc_1/2+airgap)*sin(angle_hall+pi/2),-(D_arc_1/2+airgap)*cos(angle_hall+pi/2),(D_arc_1/2+airgap)*sin(angle_hall+pi/2)+hall_depth)
mi_addsegment((D_arc_1/2+airgap)*cos(angle_hall+pi/2),(D_arc_1/2+airgap)*sin(angle_hall+pi/2)+hall_depth,-(D_arc_1/2+airgap)*cos(angle_hall+pi/2),(D_arc_1/2+airgap)*sin(angle_hall+pi/2)+hall_depth)
mi_clearselected();
mi_addblocklabel(0,(D_arc_1/2+airgap+hall_depth/2));
mi_selectlabel(0,(D_arc_1/2+airgap+hall_depth/2));
mi_setblockprop('Air', 0, 1, 'None', 0, 0, 0);
mi_selectrectangle(-(D_arc_1/2+airgap)*cos(angle_hall+pi/2),(D_arc_1/2+airgap)*sin(angle_hall+pi/2),(D_arc_1/2+airgap)*cos(angle_hall+pi/2),(D_arc_1/2+airgap)*sin(angle_hall+pi/2)+hall_depth,4)
mi_copyrotate2(0,0,-360/pole_number/2,1,4)
...
for k=1:num_iter
    mi_analyze(1)
    mi_loadsolution()
    xpoint_1=real(((D_arc_1/2)+hall_depth/2)*exp(j*(pi/2)));
    ypoint_1=imag(((D_arc_1/2)+hall_depth/2)*exp(j*(pi/2)));
    pv = mo_getpointvalues(xpoint_1,ypoint_1);
    B_cos_rot(k)=-imag((pv(2)+j*pv(3))); %In order to change the coordinates system.
    xpoint_2=real(((D_arc_1/2)+hall_depth/2)*exp(j*(-pi/6+pi/2)));
    ypoint_2=imag(((D_arc_1/2)+hall_depth/2)*exp(j*(-pi/6+pi/2)));
    pv = mo_getpointvalues(xpoint_2,ypoint_2);
    B_sin_rot(k)=-imag((pv(2)+j*pv(3))*exp(j*(pi/6))); %In order to change the coordinates system.
    mo_close()
    mi_selectcircle(0,0,D_arc_1/2+0.1,4)
    mi_selectgroup(2)
    mi_selectgroup(3)

    mi_moverotate(0,0,360/pole_number/num_iter)
    k=k+1;
end
    mi_loadsolution()
