function [bestmem,bestval,nfeval] = DEoptimize_ECCE(fname,freport_name,AlgorithmParams)

VTR=AlgorithmParams.VTR;
D = AlgorithmParams.D;
XVmin =AlgorithmParams.XVmin;
XVmax = AlgorithmParams.XVmax;
y=AlgorithmParams.y;
NP = AlgorithmParams.NP;
itermax = AlgorithmParams.itermax;
F = AlgorithmParams.F;
CR = AlgorithmParams.CR;
strategy = AlgorithmParams.strategy;
refresh = AlgorithmParams.refresh;
...
nargin=numel(fieldnames(AlgorithmParams))-1;
...
nfeval    = 1;                    % number of function evaluations

%------Evaluate the best member after initialization----------------------

ibest   = 1;                      % start with first population member
%%val(1)  = feval(fname,pop(ibest,:),nfeval,y);
nfevals = nfeval + [1:NP] - 1;
for i = 1:NP
    [val(i), rep(i)]  = feval(fname,pop(i,:),nfevals(i),y);
end
% Include structural analysis
% save temp
% for i = 1:NP
%     para=[rep(i).EM_input(i).D_rotor_outer
%         rep(i).EM_input(i).m_thickness
%         rep(i).EM_input(i).pole_arc_ratio
%         rep(i).EM_input(i).airgap];
%     [ST_output]=ST_BPMSM(para)
%     if(ST_output.SF<1.5)
%         penalty_stress=999;
%         val(i)=val(i)+penalty_stress;
%     end
%     rep(i).ST_output(i)=ST_output;
% end
...
for i=2:NP                        % check the remaining members
    %val(i) = feval(fname,pop(i,:),nfeval,y);
    nfeval  = nfeval + 1;
...
[all_solve_data] = feval(freport_name, all_solve_data, rep, nfeval);
...
while ((iter < itermax) & (bestval > VTR))
    ...
    nfevals = nfeval + [1:NP] - 1;
    for i=1:NP
        [tempvals(i), rep(i)] = feval(fname,ui(i,:),nfevals(i),y);
    end
    
%     for i = 1:NP
%        j= nfevals(i)
%         para=[rep(i).EM_input(j).D_rotor_outer
%             rep(i).EM_input(j).m_thickness
%             rep(i).EM_input(j).pole_arc_ratio
%             rep(i).EM_input(j).airgap];
%         [ST_output]=ST_BPMSM(para)
%         if(ST_output.SF<1.5)
%             penalty_stress=999;
%             tempvals(i)=tempvals(i)+penalty_stress;
%         end
%         rep(i).ST_output(j)=ST_output;
%     end    
    
    for i=1:NP
        tempval = tempvals(i); %feval(fname,ui(i,:),nfeval,y);   % check cost of competitor
        nfeval  = nfeval + 1;
