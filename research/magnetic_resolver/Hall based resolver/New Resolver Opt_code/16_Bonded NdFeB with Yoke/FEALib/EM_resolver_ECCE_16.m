function [Eval_value] = EM_resolver_ECCE_16(para,nfeval,mode)
...
% deg_arc_1=30; % should use 10~120
% deg_arc_2=30; % should use 10~40
% m_in_1=2; % should use 1~3
% slit_width=13; % should use width 5~10
% m_thickness=1; % should use 1~3
% m_gap=1;
% b_th_2=1.5; % use 1 to 3
...
x_bridge_3=((D_arc_2/2)-1-m_thickness)*sin(pi/pole_number)+(-0.5)*cos(pi/pole_number);
y_bridge_3=((D_arc_2/2)-1-m_thickness)*cos(pi/pole_number)-(-0.5)*sin(pi/pole_number);
...
mi_clearselected();
mi_addblocklabel(slit_width/2-m_gap+0.1,(D_arc_1/2)-m_thickness-m_in_1+0.1);
mi_selectlabel(slit_width/2+0.3,(D_arc_1/2)-m_thickness-m_in_1+0.1);
mi_setblockprop('Air', 0, 1, 'None', 0, 0, 0);
mi_clearselected();
...
if v_shape==0
    for z=1:2:pole_number
        ...
        mi_setblockprop('BN10',1,0,'None',30+(z-1)*360/pole_number,2,1);
        mi_clearselected();
    ...
    for z=2:2:pole_number
        ...
        mi_addblocklabel(((D_arc_1/2)-0.5*m_thickness-m_in_1)*cos((30+(z-1)*360/pole_number)*2*pi/360),((D_arc_1/2)-0.5*m_thickness-m_in_1)*sin((30+(z-1)*360/pole_number)*2*pi/360));
        mi_selectlabel(((D_arc_1/2)-0.5*m_thickness-m_in_1)*cos((30+(z-1)*360/pole_number)*2*pi/360),((D_arc_1/2)-0.5*m_thickness-m_in_1)*sin((30+(z-1)*360/pole_number)*2*pi/360));
        mi_setblockprop('BN10',1,0,'None',(30+(z-1)*360/pole_number-180),2,1);
        mi_clearselected();
