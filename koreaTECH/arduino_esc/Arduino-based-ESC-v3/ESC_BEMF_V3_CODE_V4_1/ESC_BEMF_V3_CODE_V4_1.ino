...
void setup() {  
  ...
  if( EEPROM.read(1) != 1)      //This will only run once after you uplaod the code
  {
  ...
  /*#error First time you uplaod the code make sure the next line is "uncommented". 
  Then, comment back the line and uplaod the code again and delete this entire error line*/
  //EEPROM.write(1, 0);
    ...
    while(MOTOR_SPINNING) 
    {
      ...
      if(PWM_INPUT < (PWM_IN_MIN + 30))
      {
        ...
        {          
          ...
          motor_off_counter = 0;
          PORTD = B00000000;      //Set D0-D7 to LOW        
          TCCR1A =  0;            //OC1A - D9 compare match noninverting mode, downcounting ,PWM 8-bit 
          TCCR2A =  0;            //OC1A - D9 compare match noninverting mode, downcounting ,PWM 8-bit 
